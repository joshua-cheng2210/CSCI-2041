(* These are the functions and modules I get from the module in the lib directory.
   ...
   let string_of_declaration = Proj.string_of_program
   let mainParser = Proj.Parser.main
   let mainLexer = Proj.Lexer.token
   module Parser : (sig exception Error end) = Proj.Parser
